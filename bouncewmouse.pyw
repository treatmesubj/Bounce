...
# if user doesn't execute full script, they'll be stuck with the NO cursor
# they'll just need to rerun these two lines to set it back
hCursor = win32api.LoadCursor(0, win32con.OCR_NO)
ctypes.windll.user32.SetSystemCursor(hCursor, win32con.OCR_NORMAL)
...
def movemouse():
    ...
    step = 5
    for i in range(1000):
        x = int(round(h + (200 * math.cos(theta))))
        y = int(round(k - (200 * math.sin(theta))))
        theta += step
